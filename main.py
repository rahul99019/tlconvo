...
async def start_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not user_key:
        ...
        await update.message.reply_text(
            f"*Your approval key:* `{user_key}`\n\nTo activate your bot, send this key to WhatsApp: *{WHATSAPP_CONTACT}*",
            parse_mode='Markdown'
        )
    ...
    if await is_key_approved(user_key):
        ...
        await update.message.reply_text(
            f"Your key (`{user_key}`) is not approved yet.\nContact admin on WhatsApp: *{WHATSAPP_CONTACT}*",
            parse_mode='Markdown'
        )
        context.user_data['step'] = 'waiting_for_approval'
